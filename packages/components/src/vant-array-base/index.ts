...
export interface IVantArrayBaseAdditionProps extends VantButtonProps {
  title?: string
  method?: 'push' | 'unshift'
  defaultValue?: any
}

export interface IVantArrayBaseProps {
  // disabled?: boolean
  keyMap?: WeakMap<Object, string> | string[] | null
}

export interface IVantArrayBaseItemProps {
  index: number
  record: any
}

export interface IVantArrayBaseContext {
  field: Ref<ArrayField>
  schema: Ref<Schema>
  props: IVantArrayBaseProps
  keyMap?: WeakMap<Object, string> | string[] | null
}

const ArrayBaseSymbol: InjectionKey<IVantArrayBaseContext> =
  Symbol('ArrayBaseContext')
const ItemSymbol: InjectionKey<IVantArrayBaseItemProps> = Symbol('ItemContext')
...
const ArrayBaseItem = defineComponent({
  ...
  emits: ['change', 'focus', 'blur'],
  props: ['index', 'record'],
  setup(props: IVantArrayBaseItemProps, { slots, emit }) {
...
export type VantArrayBaseMixins = {
...
export const VantArrayBase = composeExport(ArrayBaseInner, {
